...
class ACRecord(object):
    ...
    def undo(self):
        ...
        if action[0] == tx.ACTCreateModule:
            ...
            index = module.index
            self.project.circuit.rm_module_by_index(index)
            self.mw.lw.remove_module(index)
            ...
            self.project.circuit.del_device(device)
        ...
        if action[1] == 'del module':
            ...
            self.project.circuit.module_list.insert(index, module)
            #self.mw.dw.active_canvas(module.canvas)
            self.mw.aw.active_module()
            self.mw.lw.insert_module(index, module.name)
        elif action.type == 'new device':
            attr = action.aw_stack['device']
            self.hide_item_by_id(attr.did)
            self.mw.aw.cur_module.hide_device_win(attr)
            ...
            self.mw.lw.update_tag_by_id(action.lw_stack['index'], last_name)
    ...
    def forward(self):
        ...
        if action[0] == tx.ACTCreateModule:
            ...
            index = module.index
            self.mw.lw.insert_module(index, module.name)
            self.project.circuit.insert_module(index, module)
...
class ACProject(object):
    ...
    def new_project(self):
        ...
        if self.project.exist():
            self.close_project()
        self.project.new_project(self.mw.aw.cur_win.name, self.mw.aw.cur_win.pdir)
    ...
    def open_project(self):
        ...
        for module in data:
            ...
            for dev in module['wire']:
                tid = self.cur_module.canvas.create_line(dev['coord'])
            ...
            for location in module['shape']:
                pin_location.append([self.cur_module.get_pin[pin] for pin in location])
            self.cur_module.inst_view.config_pin_placement(pin_location)
...
class ACDesign(object):
    ...
    def create_template(self):
        """create a module with a template and show."""
        self.deactive_device()
        self.deactive_module()
        self.new_template()
        self.active_module()
    ...
    def create_input(self, e):
        ...
        self.active_device()
        self.record_act(tx.ACTCreateDevice, device=self.cur_device)
    ...
    def create_inst(self, e):
        ...
        self.active_device()
        self.record_act('new device', device=self.cur_device)
        lg.info(f"创建实例 {name}")
        self.end_action()
    def create_regb(self, e):
        ...
        # get module name
        data = [] # get selected module attr frame
        xv.DiaSelFile(self.mw, data, tx.DiaSelFileTitle)
        if data == []:
            return
        name = f'regb_'
        coord = (e.x, e.y)
        ...
        self.active_device()
        self.record_act('new regb', device=self.cur_device)
    ...
    def create_regfile(self, e):
        ...
        self.active_device()
        self.record_act('new regfile', device=self.cur_device)
    ...
    def create_wire(self, coord, item):
        ...
        lg.info(f"创建线 {name}")
        self.end_action()
    def new_template(self, name='template'):
        """create a module with a template."""
        self.cur_module = self.project.circuit.new_template(name, self.mw.dw.new_module())
        self.mw.lw.new_module(self.cur_module.name) # index
    def new_input(self, name, width, coord):
        """以默认名称创建输入端口"""
        self.project.circuit.new_input(name, width, coord)
    def new_output(self, name, width, coord):
        """以默认名称创建输出端口"""
        self.project.circuit.new_output(name, width, coord)
    def new_inst(self, name, coord, ref_module):
        """call create instance function"""
        self.project.circuit.new_inst(name, coord, ref_module)
    def new_regfile(self, name, coord):
        """call create instance function"""
        self.project.circuit.new_regfile(name, coord)
    def new_wire(self, name, width, coord, item):
        """在画布上确定画出线后，再创建线逻辑。"""
        self.project.circuit.new_wire(name, width, coord, item)
    def new_regb(self, name, coord, fname):
        """call create instance function"""
        with open(fname) as fin:
            regb = json.load(fin)
            self.project.circuit.new_regb(name, coord, regb)
    def delete(self):
        """删除动作。"""
        if self.cur_device.name == tx.NameNone:
            self.del_module()
        else:
            self.del_device()
    def del_module(self):
    ...
    def del_device(self):
        ...
        self.project.circuit.del_device()
        self.mw.aw.active_module(self.cur_module)
    ...
    def move_device(self):
        ...
        if isinstance(self.cur_device, circuit.LWire):
            self.bind_move_wire()
            #self.end_move_wire()
            raise NotImplementedError("移动线")
        elif self.cur_device:
            lg.debug("move device: %s", self.cur_device.name)
            start_point = self.cur_device.graph.coord
    ...
    def active_module(self, index=None):
        ...
        if index is not None:
            self.deactive_module()
            self.cur_module = self.project.circuit.get_module_by_index(index)
            #self.mw.lw.active_module_by_index(index)
        self.cur_module.active()
        self.mw.aw.active_module(self.cur_module)
        lg.debug((tx.ac.activeModule+" %s"), self.cur_module.name)
    def deactive_module(self, index=None):
        """不激活任何模块。"""
        if self.cur_module:
            self.cur_module.canvas.pack_forget()
        self.cur_module = None
        self.mw.aw.deactive_module()
    def active_device(self, item_id=None):
        """激活器件。当前器件或当前canvas选中的器件。"""
        if item_id is not None:
            if self.cur_device:
                if item_id in self.cur_device.graph.all_items:
                    return
        self.project.circuit.active_device(item_id)
        self.mw.aw.active_device(self.cur_device)
        lg.debug("激活器件: %s", self.cur_device.name)
    ...
    def init_instance_view(self):
        ...
        self.cur_module.init_instance_view()
        #self.clear_undo()
        #self.clear_forward()
    def update_module_name(self, name):
        """修改模块名称"""
        last_name = self.view.cur_module.logic.name
        if last_name == name:
            return
        # 电路逻辑修改模块名称(当前module就是要修改的)
        self.cur_module.logic.name = name
    ...
    def update_device_name(self, new_name):
        ...
        old_name = self.cur_device.name
        if old_name == new_name:
            return
        self.cur_device.name = new_name
        self.cur_device.graph.update_name()
    ...
    def update_device_width(self, new_width):
        ...
        old_width = self.cur_device.width
        if old_width == new_width:
            return
        self.cur_device.width = new_width
    ...
    def update_regfile(self, config):
        ...
        self.cur_device.update_config(config)
        self.cur_device.graph.update()
    def get_all_cp(self):
        """得到当前器件的所有连接点。"""
        return self.cur_device.graph.get_all_cp()
...
class ACEvent(object):
    ...
    def bind_mouse_default_action(self):
        ...
        self.cur_module.canvas.bind("<ButtonPress-1>", self._bind_select)
        #self.cur_module.canvas.bind("<ButtonRelease-1>", self._button_release)
        #self.cur_module.canvas.bind("<ButtonRelease-3>", self._cancle)
        #self.cur_module.canvas.bind("<B1-Motion>", self._button_motion)
        self.cur_module.canvas.bind("<ButtonPress-3>", self._null)
        self.cur_module.canvas.bind("<Motion>", self._null)
    ...
    def _bind_select(self, event):
        ...
        '''
        #self._drag_data["item"] = self.cur_module.canvas.find_closest(event.x, event.y)[0]
        #self._drag_data["item"] = self.cur_module.canvas.find_enclosed(event.x-1, event.y-1, event.x+1, event.y+1)[0]
        ids = self.cur_module.canvas.find_overlapping(event.x-5, event.y-5, event.x+5, event.y+5)
        #if isinstance(self.cur_graph, xg.GWire):
        if isinstance(self.cur_device, circuit.LWire):
            for one in ids:
                tags = self.cur_module.canvas.gettags(one)
                if tx.TagFlagPoint in tags:
                    if self.cur_graph.select_point(one):
                        # selece flag point of the wire again
                        return
        for one in ids:
            if tx.TagFlagPoint in self.cur_module.canvas.gettags(one):
    ...
    def bind_new_input(self):
        """设置鼠标行为，选择器件起始点。"""
        self._escape(None)
        self.cur_module.canvas.bind("<ButtonPress-1>", self.create_input)
    def bind_new_output(self):
        """设置鼠标行为，选择器件起始点。"""
        self._escape(None)
        self.cur_module.canvas.bind("<ButtonPress-1>", self.create_output)
    def bind_new_inst(self):
        """设置鼠标行为，选择器件起始点。"""
        self._escape(None)
        self.cur_module.canvas.bind("<ButtonPress-1>", self.create_inst)
    def bind_new_regb(self):
        """create bundle of register."""
        self._escape()
        self.cur_module.canvas.bind("<ButtonPress-1>", self.create_regb)
    def bind_new_regfile(self):
        """create bundle of register."""
        self._escape()
        self.cur_module.canvas.bind("<ButtonPress-1>", self.create_regfile)
    def bind_new_wire(self):
        """设置鼠标行为，选择起始连接点，设置新的鼠标行为。"""
        self._escape()
        self.cur_module.canvas.bind("<ButtonPress-1>", self._create_temp_wire)
    def bind_move_device(self, start_point):
        ...
        self.cur_module.canvas.bind("<Motion>", self._show_device_md)
        self.cur_module.canvas.bind("<ButtonPress-1>", self._get_end_point_md)
    def bind_move_wire(self):
        """移动线，只移动线的线段。"""
        self._escape(None)
        #self.cur_module.canvas.bind("<Motion>", self._indicate_moveable_segment)
        self.cur_module.canvas.bind("<ButtonPress-1>", self._get_start_point_mw)
    ...
    def _escape(self, event=None):
        ...
        if self.cur_state == "move_device":
            ...
            for item in self.cur_items:
                self.cur_module.canvas.move(item, dx, dy)
        for item in self.temp_item:
            self.cur_module.canvas.delete(item)
        self.temp_coord.clear()
        self.cur_state = 'idle'
        self.bind_mouse_default_action()
    # wire related
    def _create_temp_wire(self, event):
        ...
        tag_type = "type=wire"
        tags = (tag_type)
        tid = self.cur_module.canvas.create_line(tbox, tags=tags)
        self.temp_item.append(tid) # 画默认的线
        self.cur_module.canvas.bind("<ButtonPress-3>", self._get_next_point_wire)
        self.cur_module.canvas.bind("<ButtonPress-1>", self._get_end_point_wire)
        self.cur_module.canvas.bind("<Motion>", self._show_line_wire)
    def _get_next_point_wire(self, event):
        """get next cp = the second point of the temp wire """
        item = self.temp_item[-1]
        tbox = self.cur_module.canvas.coords(item)
        new_coord = tbox + [tbox[-2]+4, tbox[-1]]
        self.cur_module.canvas.coords(item, new_coord)
    ...
    def find_cp(self, x, y):
        ...
        ids = self.cur_module.canvas.find_overlapping(x-3, y-3, x+3, y+3)
        if ids:
            for one_id in ids:
                tags = self.cur_module.canvas.gettags(one_id)
                for one_tag in tags:
                    if one_tag.startswith("cp"):
                        dev_coord = self.cur_module.canvas.coords(one_id)
    ...
    def _get_end_point_md(self, event):
        ...
        for one_wire in self.md_connected_wire:
            ...
            org_coord = self.cur_module.canvas.coords(wid)
            new_coord = move_connected_wire(wid, moved_point, org_coord, delta_x, delta_y)
            self.cur_module.canvas.coords(wid, new_coord)
    ...
    def _get_dev_connected_wire(self, cp_coord):
        ...
        for one_id in self.cur_module.canvas.find_overlapping(bbox[0], bbox[1], bbox[2], bbox[3]):
            tags = self.cur_module.canvas.gettags(one_id)
            if 'type=wire' in tags:
                #wire_coord = self.cur_module.canvas.coords(one_id)
                self.md_connected_wire.append([one_id, cp_coord]) # wire id and connect point
    def _get_start_point_mw(self, e):
        """获取要移动的线段。"""
        raise NotImplementedError("移动线")
        to_move_seg = xa.get_to_move_seg((e.x, e.y), self.cur_module.canvas.coord(self.cur_items[0]))
        self.cur_module.canvas.bind("<Motion>", self._show_wire_mw)

    def close(self):
        """关闭"""
        self.cur_module.canvas = None
        self.cur_state = 'idle'
        self.temp_coord = []
        self.temp_item = []
    ...
    def hide_item_by_id(self, did):
        """hide item ty id"""
        # TODO: remove.
        for item in self.cur_module.canvas.find_withtag(xg.tag_did(did)):
            self.cur_module.canvas.itemconfig(item, state=tk.HIDDEN)
...
class ACTNewModule(ACType):
    ...
    def undo(self, ac):
        ...
        ac.circuit.rm_module_by_index(index)
        ac.lw.remove_module(index)
        ac.deactive_module()
        lg.info(f"{tx.InfoCancelCreateDevice}: {self.module.name}")
    def redo(self, ac):
        index = self.module.index
        self.mw.lw.insert_module(index, self.module.name)
        self.project.circuit.insert_module(index, self.module)
        lg.info(f"{tx.InfoCreateModule}: {self.module.name}")

class BAction(ACRecord, ACProject, ACEvent, ACDesign):
    """docstring for ACDesign
    ...
    * del_module = [id, 'del module', module, index, command]
    ...
    def cur_items(self):
        ...
        return self.cur_device.graph.items
    @property
    def cur_graph(self):
        """当前器件的图形."""
        return self.cur_device.graph

class Action(ACProject, ACDesign):
    ...
    def init_view(self, view):
        self.view = view(self)
        self.mw = self.view.mw
        self.view()
    def new_module(self, name='module'):
        """以默认名称创建模块"""
        md = self.project.circuit.new_module(name)
        return self.view.new_module(md)
